...
[docs]def cross_entropy(output, target, name=None):
...
[docs]def sigmoid_cross_entropy(output, target, name=None):
...
[docs]def binary_cross_entropy(output, target, epsilon=1e-8, name='bce_loss'):
...
[docs]def mean_squared_error(output, target, is_mean=False, name="mean_squared_error"):
...
[docs]def normalized_mean_square_error(output, target, name="normalized_mean_squared_error_loss"):
...
[docs]def absolute_difference_error(output, target, is_mean=False, name="absolute_difference_error_loss"):
...
[docs]def dice_coe(output, target, loss_type='jaccard', axis=(1, 2, 3), smooth=1e-5):
...
[docs]def dice_hard_coe(output, target, threshold=0.5, axis=(1, 2, 3), smooth=1e-5):
...
[docs]def iou_coe(output, target, threshold=0.5, axis=(1, 2, 3), smooth=1e-5):
...
[docs]def cross_entropy_seq(logits, target_seqs, batch_size=None):  # , batch_size=1, num_steps=None):
...
[docs]def cross_entropy_seq_with_mask(logits, target_seqs, input_mask, return_details=False, name=None):
    """Returns the expression of cross-entropy of two sequences, implement
...
[docs]def cosine_similarity(v1, v2):
...
[docs]def li_regularizer(scale, scope=None):
...
[docs]def maxnorm_regularizer(scale=1.0):
...
[docs]def maxnorm_o_regularizer(scale):
...
[docs]def maxnorm_i_regularizer(scale):
